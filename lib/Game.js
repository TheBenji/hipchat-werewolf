...
Game.prototype.help = function () {
    return this.eventEmitter.emit(ROOM_NOTIFICATION, "/start to start game" +
        "\n/join to join game" +
        "\n/time to see how much time left in phase" +
        "\n/alive to see living people" +
        "\n/kill @mention name to vote for person during day" +
        "\nAs wolf @mention name in private message to Moderator to kill person during night ", config.roomId);
};
...
Game.prototype.time = function () {
    if (this.gamePhase != 0) {
        ...
        return this.eventEmitter.emit(ROOM_NOTIFICATION, "You have " + timeLeft + " minutes left this phase", config.roomId);
    }
};
