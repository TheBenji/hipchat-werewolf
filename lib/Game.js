...
Game.prototype.mainLoop = function () {
    var gameResult = this.checkWinConditions();
    if (gameResult == 2) {
        this.eventEmitter.emit("roomNotification", "Wolfs won!", config.roomId);
        this.cleanUp();
        return;
    } else if (gameResult == 1) {
        this.eventEmitter.emit("roomNotification", "Villagers won!", config.roomId);
        this.cleanUp();
        return;
    }
    ...
    if (this.gamePhase == 2 && this.startedTimeStamp + config.durations.night < Date.now()) {
    ...
    if (this.gamePhase == 5 && this.startedTimeStamp + config.durations.day < Date.now()) {
        var result = this.checkVotes();
        if (result) {
            player = this.findPlayerById(result);
            player.dead = true;
