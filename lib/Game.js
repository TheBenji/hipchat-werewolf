...
Game.prototype.assignPlayers = function () {
    ...
    while (assignedWolfs.length != config.rolesCount.wolf) {
        ...
        if (!this.players[position].role) {
            ...
            assignedWolfs.push(this.players[position].id);
        }
    }

    this.eventEmitter.emit("createRoomAndInvite", "wolfs","Pick person to kill by specifying mention name!", assignedWolfs);
...
Game.prototype.cleanUp = function () {
    this.players = [];
    this.gamePhase = 0;
    this.eventEmitter.emit("deleteRoom", this.wolfRoomId);
    this.wolfRoomId = -1;
